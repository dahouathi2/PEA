...
class Dataset_ean(Dataset):
    ...
    def inverse_transform(self, data):
        return self.scaler.inverse_transform(data)
"""
class Dataset_few_shot_seq(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h', percent=100,
                 seasonal_patterns=None, df = None):
        if size == None:
            self.seq_len = 24 * 4 * 4
            self.label_len = 24 * 4
            self.pred_len = 24 * 4
            ...
            self.pred_len = size[2]
        # init
        assert flag in ['train', 'test', 'val', 'few_shot']
        type_map = {'train': 0, 'val': 1, 'test': 2, 'few_shot': 3}
        self.set_type = type_map[flag]
    ...
    def __read_data__(self,df):
        ...
        self.mean = 0
        self.std = 1
        df_raw = df[["end_date", "sold_units", "price_range", "seasonality_index"]]
        #df_raw.drop("sub_tactic", axis=1, inplace=True)# don't forget to remove this once regulated the data
        df_raw.rename(columns={'end_date': 'date'}, inplace=True)
        '''
        df_raw.columns: ['date', ...(other features), target feature]
        '''
        cols = list(df_raw.columns)
...
class Dataset_ean(Dataset):
    ...
    def __read_data__(self):
        self.scaler = StandardScaler()
        df_raw = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path))[["end_date", "sold_units", "price_range", "seasonality_index"]]
        #df_raw.drop("sub_tactic", axis=1, inplace=True)# don't forget to remove this once regulated the data
        df_raw.rename(columns={'end_date': 'date'}, inplace=True)
        '''
        df_raw.columns: ['date', ...(other features), target feature]
...
class Dataset_Pretrain(Dataset):
    ...
    def __read_data__(self):
        self.scaler = StandardScaler()
        df_raw = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path)).drop('family', axis=1)
